...
def parse_args():
    ...
    p.add_argument('--dataset', type=str, default='MIntRec',
    ...
    p.add_argument('--pretrain_path', type=str, default=None,
                   help='Path to saved pretrain weights')
...
def main():
    ...
    for k, v in cfg.items():
        if k == 'batch' and args.batch_size is not None: continue
        if k == 'grad_acc_steps' and args.grad_acc_steps is not None: continue
        setattr(args, k, v)
    if args.batch_size is not None: args.batch = args.batch_size
    if not hasattr(args, 'grad_acc_steps'): args.grad_acc_steps = 1
